void handle_water(ivec3 index) {
    ...
    for (int i = -1; i <= 1; i++) {
        ...
        for (int k = -1; k <= 1; k++) {
            ...
            ivec3 side_block_index = ivec3(left_index, index.y, right_index);
    ...
    // If we can't go anywhere, preserve the block in its current position
